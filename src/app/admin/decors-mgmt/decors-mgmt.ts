  ...
  template: `
    <div class="max-w-6xl mx-auto space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-heading font-bold text-text-primary">Decors Management</h1>
          <p class="text-text-muted mt-1">Manage your decor offerings</p>
        </div>
        <button class="btn-primary flex items-center gap-2" (click)="openModal()">
          <app-icon name="plus" [size]="18"></app-icon> Add Decor
        </button>
      </div>
      ...
      <div class="bg-bg-surface border border-border rounded-2xl overflow-hidden shadow-card">
        @if (isLoading()) {
          <div class="p-8 text-center text-text-muted">Loading decors...</div>
        } @else if (decors().length === 0) {
          <div class="p-8 text-center text-text-muted">No decors found. Click "Add Decor" to create one.</div>
        } @else {
          <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
              ...
              <tbody class="divide-y divide-border">
                @for (item of decors(); track item.id; let i = $index) {
        ...
        <div class="bg-bg-surface border border-border rounded-2xl w-full max-w-lg shadow-elevated overflow-hidden animate-fade-in-up my-auto">
          ...
          <div class="px-6 py-4 border-b border-border flex items-center justify-between bg-bg-elevated/50">
            <h2 class="text-lg font-heading font-semibold text-text-primary">
              {{ editingItem() ? 'Edit Decor' : 'New Decor' }}
            </h2>
...
export class DecorsMgmtComponent implements OnInit {
  private api = inject(ApiService);
  private cropper = inject(ImageCropperService);
  
  items = signal<EventDecorItem[]>([]);
  decors = computed(() => this.items().filter(i => i.type === 'decor').sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0)));
  ...
  loadItems() {
    ...
    // Fetch all, filter happens in computed 'decors'
  ...
  openModal() {
    ...
    this.form = {
      ...
      type: 'decor',
      isActive: true,
      sortOrder: this.decors().length + 1
    };
    this.clearPreview();
  ...
  async saveItem(): Promise<void> {
    ...
    const payload = {
      ...
      imageUrl,
      type: 'decor' as const, // Force decor type
