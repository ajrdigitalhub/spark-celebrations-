...
@Component({
  selector: 'app-event-decors-mgmt',
  standalone: true,
  ...
  template: `
    <div class="max-w-6xl mx-auto space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-heading font-bold text-text-primary">Events & Decors Management</h1>
          <p class="text-text-muted mt-1">Manage event types and decor options</p>
        </div>
      ...
      <div class="bg-bg-surface border border-border rounded-2xl overflow-hidden shadow-card">
        @if (isLoading()) {
          <div class="p-8 text-center text-text-muted">Loading items...</div>
        } @else if (items().length === 0) {
          <div class="p-8 text-center text-text-muted">No items found. Click "Add New" to create one.</div>
        } @else {
                  ...
                  <tr class="hover:bg-bg-elevated/20 transition-colors">
                    ...
                    <td class="p-4">
                      <span class="px-2 py-1 text-xs font-medium rounded-md uppercase" 
                            [class.bg-accent-subtle]="item.type === 'event'"
                            [class.text-accent]="item.type === 'event'"
                            [class.bg-purple-500]="item.type === 'decor'"
                            [class.bg-opacity-20]="item.type === 'decor'"
                            [class.text-purple-400]="item.type === 'decor'">
                        {{ item.type }}
                      </span>
                    </td>
                    <td class="p-4">
          ...
          <form (ngSubmit)="saveItem()" class="p-6 space-y-4">
            ...
            <div class="flex gap-4">
              <label class="flex items-center gap-2 cursor-pointer">
                <input type="radio" [(ngModel)]="form.type" name="type" value="event" class="text-accent focus:ring-accent bg-bg-primary border-border">
                <span class="text-sm text-text-primary">Event</span>
              </label>
              <label class="flex items-center gap-2 cursor-pointer">
                <input type="radio" [(ngModel)]="form.type" name="type" value="decor" class="text-accent focus:ring-accent bg-bg-primary border-border">
                <span class="text-sm text-text-primary">Decor</span>
              </label>
            </div>
...
export class EventDecorsMgmtComponent implements OnInit {
  ...
  loadItems() {
    ...
    this.api.getEventDecors(true).subscribe({
      next: (data) => {
        this.items.set(data);
  ...
  async saveItem(): Promise<void> {
    if (!this.form.title || !this.form.type) return;
